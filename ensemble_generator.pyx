...
cpdef double generate_initial_active_noise(float tau_c, float d_ne, int model):
    ...
    cdef double noise_variance = d_ne * tau_c**(1-2*n)
    cdef double initial_noise = np.random.normal(0, np.sqrt(noise_variance))
...
cpdef cnp.ndarray[cnp.double_t, ndim=2] x_euler_method(float tau_c, float d_ne, float gain,
    ...
    time = 0
    zeta = generate_initial_active_noise(tau_c, d_ne, model)
    x_array[0] = - delta_g
    lmb_array[0] = 0
