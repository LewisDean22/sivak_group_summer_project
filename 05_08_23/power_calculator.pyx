...
cpdef cnp.ndarray[cnp.double_t, ndim=2] generate_power_ensembles(
    ...
    cdef double x
    cdef cnp.ndarray[cnp.double_t, ndim=3] truncated_trajectories
    ...
    for sample_index, trajectory in enumerate(trajectory_ensemble):
        truncated_trajectory = trajectory[:, transient_steps:]
        x_array, lmb_array = truncated_trajectory[0], truncated_trajectory[1]
...
cpdef double standard_error_on_mean(
    cnp.ndarray[cnp.double_t, ndim=1] power_ensemble):
    '''
    ddof=1 in np.std() uses the Bessel correction for sample
    variance.
    '''
    
    cdef cnp.ndarray[cnp.double_t, ndim=2] power_ensemble_samples = (
        np.reshape(power_ensemble, (1, len(power_ensemble))))
    
    mean_distribution = bootstrap(power_ensemble_samples, np.mean,
                                  n_resamples=1000)
